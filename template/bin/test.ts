...
export async function test(): Promise<void> {
  ...
  try {
    const { profile, stage, env } = await getAppConfig();
    ...
    if (IS_GITHUB) {
      ...
      process.env.AWS_ACCESS_KEY_ID = stage === 'prod' ? PROD_ACCESS_KEY_ID :
        stage === 'qa' ? QA_ACCESS_KEY_ID :
          DEV_ACCESS_KEY_ID;

      process.env.AWS_SECRET_ACCESS_KEY = stage === 'prod' ? PROD_SECRET_ACCESS_KEY :
        stage === 'qa' ? QA_SECRET_ACCESS_KEY :
          DEV_SECRET_ACCESS_KEY;
    ...
    spawn(`jest --coverage ${isVerbose} ${isWatching} --passWithNoTests`);
  } catch (error) {
    console.error(error);
    process.exit(1);
  }
